    ...
    useEffect(() => {
        ...
        .then(res => {
            ...
            res.docs.forEach(doc => {
                rejectedUsers.push({id: doc.id, ...doc.data()});
            })
            setRejectedlist(rejectedUsers);
        })
    }, [])
                                        ...
                                        <tbody>
                                            {
                                                rejectedList.map(rejected => {
                                                    return (
                                                        <tr key={rejected.id}>
                                                            <td className='va-center'>{rejected.createdAt.toDate().toLocaleDateString()}</td>
                                                            <td className='text-capitalize va-center text-nowrap'>{rejected.fullname.firstname}</td>
                                                            <td className='text-capitalize va-center text-nowrap'>{rejected.fullname.lastname}</td>
                                                            <td className='text-capitalize va-center text-nowrap'>{rejected.apply.job}</td>
                                                            <td>
                                                                <Link to={`/Admin/User/View/id/${rejected.id}`} className="btn btn-primary">
